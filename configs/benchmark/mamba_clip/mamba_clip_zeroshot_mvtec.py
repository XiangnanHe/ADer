...
class cfg(cfg_common, cfg_dataset_default, cfg_model_mamba_clip_zeroshot):

    def __init__(self):
        cfg_common.__init__(self)
        cfg_dataset_default.__init__(self)
        cfg_model_mamba_clip_zeroshot.__init__(self)
        ...
        # Training settings (optional - for adapter fine-tuning)
        # Can skip training and use pure zero-shot by running test only
        self.epoch_full = 20  # Fewer epochs - only for adapter fine-tuning
        ...
        # Optimizer settings (only for optional adapter fine-tuning)
        self.lr = 0.0001
        ...
        self.model.kwargs = dict(
            ...
            freeze_encoder=True,  # Only train adapters if fine-tuning
        )
        ...
        # Optimizer (for optional adapter fine-tuning)
        ...
        # Trainer
        self.trainer.name = 'MambaCLIPZeroShotTrainer'
        ...
        # Loss configuration (for optional adapter fine-tuning)
        self.loss.loss_terms = [
            dict(type='CrossEntropyLoss', name='ce', lam=1.0),  # Contrastive loss
        ]
