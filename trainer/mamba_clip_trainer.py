"""
Trainer for Mamba-CLIP Anomaly Detection Model

Handles:
- Training with image-level and pixel-level losses
...
class MambaCLIPTrainer(BaseTrainer):
    def __init__(self, cfg):
        super(MambaCLIPTrainer, self).__init__(cfg)
    ...
    def forward(self):
        ...
        self.img_scores, self.pixel_maps = self.net(self.imgs)

    def optimize_parameters(self):
        """Training step with combined image and pixel-level losses"""
        # Apply mixup if configured
        if self.mixup_fn is not None:
            self.imgs, self.imgs_mask = self.mixup_fn(
                self.imgs,
                self.imgs_mask
            )
        ...
        with self.amp_autocast():
            ...
            # Image-level loss (BCE)
            # Create target: anomaly label (0 or 1) for each image
            target_img = (self.imgs_mask.reshape(self.bs, -1).max(dim=1)[0] > 0).float().unsqueeze(1)
            loss_img = self.loss_terms['bce_img'](self.img_scores, target_img)

            # Pixel-level loss (BCE)
            # Resize pixel maps to match mask size if needed
            if self.pixel_maps.shape[-2:] != self.imgs_mask.shape[-2:]:
                import torch.nn.functional as F
                pixel_maps_resized = F.interpolate(
                    self.pixel_maps,
                    size=self.imgs_mask.shape[-2:],
                    mode='bilinear',
                    align_corners=False
                )
            else:
                pixel_maps_resized = self.pixel_maps

            loss_pixel = self.loss_terms['bce_pixel'](
                pixel_maps_resized,
                self.imgs_mask
            )

            # Total loss
            total_loss = loss_img + loss_pixel
        ...
        update_log_term(
            self.log_terms.get('img_loss'),
            reduce_tensor(loss_img, self.world_size).clone().detach().item(),
            1, self.master
        )
        update_log_term(
            self.log_terms.get('pixel_loss'),
            reduce_tensor(loss_pixel, self.world_size).clone().detach().item(),
            1, self.master
        )
